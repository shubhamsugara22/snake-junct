...
content = content.replace(/\\s*\\/\\/ Draw super cute chibi character with enhanced animations[\\s\\S]*?\\/\\/ Animation: Squash and stretch based on velocity/, '\\n' + playerRenderReplacement);

const endChibiReplacement = `      if (ctx.restore) { try { ctx.restore(); } catch(e) {} }
    }
    // Draw snakes (or Halloween creatures)`;
content = content.replace(/\\s*\\/\\/ Draw snakes \\(or Halloween creatures\\)/, '\\n' + endChibiReplacement);
