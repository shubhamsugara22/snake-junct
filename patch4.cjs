...
function replaceExact(search, replacement) {
  ...
  if (index !== -1) {
    content = content.substring(0, index) + replacement + content.substring(index + search.length);
  } else {
    console.error('Could not find string to replace: ' + search.substring(0, 50) + '...');
  }
}
...
// 7. Replace player rendering (find the exact start and end blocks)
const pStartStr = `    // Draw super cute chibi character with enhanced animations`;
const pEndStr = `    // Animation: Squash and stretch based on velocity`;

const pStartIndex = content.indexOf(pStartStr);
const pEndIndex = content.indexOf(pEndStr);

if (pStartIndex !== -1 && pEndIndex !== -1) {
  content = content.substring(0, pStartIndex) +
    ...
    content.substring(pEndIndex);
} else {
  console.error("Could not find player render start/end");
}
